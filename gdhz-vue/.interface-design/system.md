...
> 最后更新：2026-02-20
> 提取自：`variables.css`、`animations.css`、`RightSidebar.vue`、`RealtimeDataPanel.vue`、`AISituationSummary.vue`
...
| 预测趋势（非 AI 侧栏） | `#3b82f6`（蓝系） | 标题 + 顶部发光线 |
| AI 预测语义（仅 AI 模块） | `#8b5cf6`（紫系） | 预测线/AI 标签差异色（不用于通用面板标题） |
...
| 微标签 | 10px | 500-600 | 站点名、角标、`risk-badge`、时间线辅助信息 |

> 约束：侧栏中的 `8px/9px` 标签已统一提升到 `10px`，正文保持 `>=12px`。
...
| `pulse-icon` | 预警图标缩放 | 改为新预警出现时触发 1 次（`0.7s`） |
| `sidebar-risk-pulse` | 侧栏红色呼吸 | 改为高风险切换时触发 3 次后停止 |
| `pulse-risk`（AI） | AI 风险图标脉冲 | 改为高风险切换时触发 3 次后停止，降低光晕强度 |
| `marquee-scroll` | 跑马灯滚动 | 保留 |

### 波纹动画（回滚基线）

- 右栏潮位/海浪波纹采用回滚后的基线方案：`tideSlide` + `chopWave`
- 本次未写入被回滚的实验性潮位/海浪关键帧参数
- 频次策略：有限循环（3 次）以降低持续干扰

### 无障碍

- 全局 `prefers-reduced-motion` 已接入 `animations.css`
- 关键组件补充局部兜底：`RightSidebar.vue`、`AISituationSummary.vue`、`RealtimeDataPanel.vue`
...
- 观测线：`#10b981`（绿，实线）
- 预测线（右栏业务图）：`#3b82f6`（蓝，虚线）
- 预测线（AI 模块）：`#8b5cf6`（紫，虚线）
- 增水柱：`#f97316`（橙，渐变柱）
- 四色警戒线：红/橙/黄/蓝虚线

### 列表折叠模式（新）

- 长列表默认只展示前 `1-2` 项，使用 `list-toggle` 展开全部
- 适用组件：潮位预警站点、浪高预警站点、受影响取水口、监测井
- 样式基线：`10px` 文本 + 虚线边框 + hover 蓝色反馈
...
1. **间距令牌落地替换**：将组件内散落的魔法数字逐步替换为 `--space-*`
2. **正文字号统一到 12px 基线**：当前仍有少量 10px 信息文本，可按模块分级提升
3. **演示化交互清理（P3）**：`alert()` / `console.log()` 替换为真实流程
4. **响应式与固定定位治理（P3）**：按断点策略将固定定位收敛为容器化布局
