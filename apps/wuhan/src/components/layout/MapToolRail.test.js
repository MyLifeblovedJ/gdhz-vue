...
describe('MapToolRail structure', () => {
  ...
  it('uses readable Chinese labels for the three tool-rail buttons', () => {
    expect(source).toContain('title="图层控制"')
    expect(source).toContain('>图层控制</span>')
    expect(source).toContain('title="观测设备"')
    expect(source).toContain('>观测设备</span>')
    expect(source).toContain('title="AI 智能助手"')
    expect(source).toContain('>智能助手</span>')
  })
})
