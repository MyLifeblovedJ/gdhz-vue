...
describe('AppHeader global emphasis', () => {
  ...
  it('keeps the header fixed while the integrated surface expands as one piece', () => {
    ...
    expect(source).toMatch(/\.mega-menu-band\s*\{[\s\S]*top:\s*calc\(100%\s*\+\s*8px\);/)
    expect(source).toMatch(/\.mega-menu-grid\s*\{[\s\S]*padding-top:\s*10px;/)
  })
  ...
  it('opens the shared submenu rail for every top-level nav item, even without children', () => {
    expect(source).toMatch(/function openMegaMenu\(\)\s*\{[\s\S]*isMegaMenuOpen\.value = true/)
    expect(source).not.toMatch(/function openMegaMenu\(item\)/)
